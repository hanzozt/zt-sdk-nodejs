 ...
 * 
 */
napi_value _zt_write(napi_env env, const napi_callback_info info) {
  napi_status status;
  size_t argc = 3;
  napi_value args[3];
  ...
  ZITI_NODEJS_LOG(DEBUG, "call zt_write");
  zt_write(conn, chunk, bufferLength, on_write, NULL);
  ZITI_NODEJS_LOG(DEBUG, "back from zt_write");
 ...
 * 
 */
void expose_zt_write(napi_env env, napi_value exports) {
  napi_status status;
  napi_value fn;

  status = napi_create_function(env, NULL, 0, _zt_write, NULL, &fn);
  if (status != napi_ok) {
    napi_throw_error(env, NULL, "Unable to wrap native function '_zt_write");
  }

  status = napi_set_named_property(env, exports, "zt_write", fn);
  if (status != napi_ok) {
    napi_throw_error(env, NULL, "Unable to populate exports for 'zt_write");
  }
